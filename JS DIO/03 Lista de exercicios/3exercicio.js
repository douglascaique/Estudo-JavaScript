/*
3) Elabore um algoritmo que calcule o que deve ser pago por um produto, considerando o preço normal de etiqueta e a escolha da condição de pagamento. 
Utilize os códigos da tabela a seguir para ler qual a condição de pagamento escolhida e efetuar o cálculo adequado.

Código Condição de pagamento:
- À vista Débito, recebe 10% de desconto;
- À vista no Dinheiro ou PIX, recebe 15% de desconto;
- Em duas vezes, preço normal de etiqueta sem juros;
- Acima de duas vezes, preço normal de etiqueta mais juros de 10%;
*/


const numero = 10;
const numeroEmTexto = '10';
console.log(numero === numeroEmTexto);

const preco = 100;
const formaDePagamento = 4;

const dezPorcento = preco * 0.1;
const desconto15 = preco * 0.15;
const juros10 = dezPorcento;




if (formaDePagamento == 1 /*debito*/) {
    pagDebito = preco - dezPorcento;
    console.log(`O valor a ser pago é R$ ${pagDebito}`)
}
else if (formaDePagamento == 2 /*Dinheiro ou Pix */) {
    pagPix = preco - desconto15
    console.log(`O valor a ser pago é R$ ${pagPix}`)
}
else if (formaDePagamento == 3 /*Em 2 vezes no credito */) {
    console.log(`O valor a ser pago é R$ ${preco}`)
}
else if (formaDePagamento == 4 /*Credito mais de 2 vezes*/) {
    pagCred = preco + juros10
    console.log(`O valor a ser pago é R$ ${pagCred}`)
}


